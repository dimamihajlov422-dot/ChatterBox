...
wss.on("connection", (ws) => {
    ...
    ws.send(JSON.stringify({ type: "history", data: history.slice(-200) }));
    ws.send(JSON.stringify({ type: "users", users: Array.from(users.values()) }));
    ...
    ws.on("message", (raw) => {
        ...
        if (msg.type === "get_private_history") {
            ...
            ws.send(JSON.stringify({ type: "private_history", with: targetNick, data: historyPrivate }));
            return;
        }
...
server.listen(3000, () => console.log("Server with private chats running on port 3000"));
